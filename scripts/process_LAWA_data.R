...
path <- './data/raw_data/lakewaterquality_monitoringdata_statetrendresults_sept2022.xlsx'
...
wqdata <- LakeWQMonitoringData

# clean up and filter the data
wqdata <- wqdata %>% 
  filter(Region=='bay of plenty') %>% 
  select(SiteID, LakeType:Units)

wqdata <- wqdata %>% 
  mutate(original_site = SiteID) %>% 
  separate(SiteID, c('blank', 'lake', 'blank2', 'blank3', 'site', 'blank4')) %>% 
  select(-c('blank', 'blank2', 'blank4', 'Symbol'))

# fix Okawa Bay issue
wqdata <- wqdata %>% 
  mutate(site = ifelse(blank3!='Site', blank3, site)) %>% 
  select(-c('blank3', 'original_site', 'Units'))

colnames(wqdata) <- c('lake', 'site', 'laketype', 'variable', 'date', 'value')

# remove dups
wqdata <- wqdata %>% 
  distinct(lake, laketype, site, date, variable, .keep_all = TRUE)

# remove erroneous datapoint where secchi is 28m in Rotoehu which is only 13m deep
wqdata$value[wqdata$variable=='Secchi' & wqdata$lake=='Rotoehu' & wqdata$date==as.Date('2020-11-16')] <- NA
