...
out %>% 
  filter(id_covar %in% c('epi_temp', 'temp_C_bottom', 'PAR_umolm2s_top',
                         ...
                         'schmidt_stability', 'DO_sat_bottom')) %>% 
...
out_prop %>% 
  filter(id_covar %in% c('epi_temp', 'temp_C_bottom', 'Kd',
                         'DRP_mgm3_bottom', 'NH4_mgm3_bottom', 'NNN_mgm3_bottom',
                         'schmidt_stability', 'DO_sat_bottom')) %>% 
...
out_prop %>% 
  filter(id_covar %in% c('epi_temp', 'temp_C_bottom', 'Kd', 'PAR_umolm2s_top',
                         'DRP_mgm3_bottom', 'NH4_mgm3_bottom', 'NNN_mgm3_bottom',
                         'schmidt_stability', 'DO_sat_bottom')) %>% 
...
out_prop %>% 
  filter(id_covar %notin% c('none', 'Kd'),
         lake %notin% 'Okaro') %>% 
  ggplot(aes(x = id_covar, y = diff_from_none, fill = id_covar)) +
  geom_boxplot() +
  scale_fill_brewer(palette = 'Set3') +
  facet_wrap(~mixing_state, scales = 'free') +
  theme_bw() +
...
out %>% 
  filter(covar %in% test_vars) %>% 
  filter(id_covar %in% c('epi_temp', 'temp_C_bottom', 'PAR_umolm2s_top', 'Kd',
                         'DRP_mgm3_bottom', 'NH4_mgm3_bottom', 'NNN_mgm3_bottom',
                         'schmidt_stability', 'DO_sat_bottom')) %>% # get rid of met variables for nowg
